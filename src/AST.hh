...
public:
  ...
  [[nodiscard]] auto isLValue() const -> bool {
    return IsLValue;
  }

  virtual auto setIsLValue(bool isLValue) -> void {
    IsLValue = isLValue;
  }
...
public:
  ...
  auto accept(ASTVisitor* V) -> void override {
    V->Visit(this);
  }
...
public:
  BinaryExpr(
      ...
      Expr(Kind ::BinaryExprK),
      TheBinaryOperator(TheBinaryOperator),
      LHS(std::move(LHS)),
      RHS(std::move(RHS)) {}
...
public:
  explicit LiteralIntegerExpr(int Val) :
      LiteralExpr(Kind::LiteralIntegerExprK),
      Val(Val) {}
...
public:
  explicit LiteralFloatExpr(float Val) :
      LiteralExpr(Kind::LiteralFloatExprK),
      Val(Val) {}
...
public:
  explicit LiteralCharExpr(char Char) :
      LiteralExpr(Kind::LiteralCharExprK),
      Char(Char) {}
...
public:
  explicit ExprStmt(std::unique_ptr<Expr> Expr) :
      Statement(Kind::ExprStmtK),
      TheExpr(std::move(Expr)) {}
...
public:
  FunctionDecl(
      ...
      Declaration(ReturnType, Kind::FunctionDeclK),
      Name(std::move(Name)),
  ...
  [[nodiscard]] auto isPrototype() const -> bool {
    return Body == nullptr;
  }
