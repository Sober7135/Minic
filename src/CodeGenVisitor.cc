...
auto CodeGenVisitor::visitArrayConstantInitilizer(
    llvm::Type* Type,
    Initializer* Init
) -> llvm::Constant* {
  // check size
  ...
  return llvm::ConstantArray::get(
      llvm::ArrayType::get(Type, Init->Children.size()),
      List
  );
}
...
auto CodeGenVisitor::init(
  ...
  for (size_t i = 0, end = InitList.size(); i != end; ++i) {
    ...
    // TODO refactor
    idxList[0] = llvm::ConstantInt::getIntegerValue(
        LW->getType(DataType::Int),
        llvm::APInt(32, 0)
    );
...
auto CodeGenVisitor::Visit(ASTNode* Node) -> void {
  Node->accept(this);
}

auto CodeGenVisitor::Visit(Declaration* Node) -> void {
  Node->accept(this);
}
...
auto CodeGenVisitor::Visit(FunctionDecl* Node) -> void {
  ...
  for (auto& Arg : F->args()) {
    ...
    auto* Alloca = LLVMWrapper::CreateEntryBlockAlloca(
        F,
        Arg.getType(),
        Arg.getName().str()
    );
...
auto CodeGenVisitor::Visit(Expr* Node) -> void {
  Node->accept(this);
}
  ...
  switch (UnaryOp) {
    ...
    case Minic::UnaryOperator::Minus:
      if (Ty->isFloatTy()) {
        TheValue = LW->Builder->CreateFSub(
            llvm::ConstantFP::get(Ty, llvm::APFloat(0.0)),
            Expr,
            "iunaryminus"
        );
      } else {
        TheValue = LW->Builder->CreateSub(
            llvm::ConstantInt::getIntegerValue(Ty, llvm::APInt(32, 0)),
            Expr,
            "funaryminus"
        );
      }
    ...
    case Minic::UnaryOperator::LogicalNot:
      ...
      TheValue = LW->Builder->CreateXor(
          Expr,
          llvm::ConstantInt::getBool(*LW->Ctx, true),
          "logicalnot"
      );
      break;
    default:
...
auto CodeGenVisitor::Visit(BinaryExpr* Node) -> void {
  ...
  if (BinOp == BinaryOperator::Assign) {
    ...
    if (!Ty) {
      Ty =
          getArrayValueType(dynamic_cast<ArraySubscriptExpr*>(Node->LHS.get()));
    }
...
auto CodeGenVisitor::Visit(LiteralExpr* Node) -> void {
  Node->accept(this);
}
