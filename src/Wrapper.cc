...
auto LLVMWrapper::load(llvm::Value*& Val, [[maybe_unused]] llvm::Type* Type)
  ...
  if (llvm::isa<llvm::AllocaInst>(Val)) {
    Val = Builder->CreateLoad(
        llvm::dyn_cast<llvm::AllocaInst>(Val)->getAllocatedType(),
        Val
    );
    return;
  } else if (llvm::isa<llvm::GlobalVariable>(Val)) {
    Val = Builder->CreateLoad(
        llvm::dyn_cast<llvm::GlobalVariable>(Val)->getValueType(),
        Val
    );
    return;
  } else {
